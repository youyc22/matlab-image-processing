function [re_image,re_info] = decode_find_3(DC_output, AC_output, width,height,QTAB,DCTAB,ACTAB,ratio,indices)
    ...
    re_info=zeros(width*height/ratio,1);
    re_info_0 = dec2bin(im_re);
    for i = 1:w*h
        for j = 1:ceil(64/ratio)
            re_info((i-1)*64/ratio+j) = re_info_0(64*(i-1)+indices(j),end);
        end
    end
